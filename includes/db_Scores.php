...
function wp_LC_create_table()
{
    ...
    SendSQL($sql);
    $sql = "ALTER TABLE $tablename ADD score DOUBLE(20, 2)";
    SendSQL($sql);
}
function wp_LC_Set_Score_to_User($userid, $score)
{
    global $table_prefix;
    $tablename = $table_prefix . "LC_Scores";
...
function wp_LC_Add_old_purchases_score()
{
    ...
    if ($result->num_rows > 0) { // condition 1
        while ($row = $result->fetch_assoc()) {
            ...
            wp_LC_Set_Score_to_User($userid, wp_LC_ConvertTo_Score($total_purchase));
        }
    }
}

function wp_LC_create_user($username, $password, $score = 0)
{
...
function wp_LC_Get_User_Score($userid)
{
    global $table_prefix;
    $tablename = $table_prefix . "LC_Scores";
...
function wp_LC_Add_Score_ToUser($userid, $score)
{
    global $table_prefix;
    $tablename = $table_prefix . "LC_Scores";
    $userScore = wp_LC_get_Score($userid) + $score;
    $sql = "UPDATE $tablename SET score = $userScore Where ID = $userid";
    SendSQL($sql);
}
